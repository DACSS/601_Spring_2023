...
library(readxl)
birds <- read_xlsx("PopEsts ProvState 2021.02.05.xlsx")
str(birds)
...
#drop if Country = CAN, then -c country

birds <- subset(birds, Country != "CAN")
birds <- subset(birds, select = -c(Country))

states <- read_csv("State statistics.csv")

#dropping variables that are unlikely to be useful for analysis, for the sake of neatness
states <- subset(states, select = -c(pop2022, pop2020, pop2019, pop2010, growthRate, growth, growthSince2010, fips))
