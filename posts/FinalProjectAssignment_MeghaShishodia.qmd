...
```{r}
index <- index[!grepl("_", index$location_key),]
```

Next, we merge our datasets and omit duplicates and corrupt entries, if any. 

```{r}
df = merge(index, health, by = "location_key", all = FALSE)
df = merge(df, vaccinations, by = "location_key", all = FALSE)
df = merge(df, epidemiology, by = "location_key", all = FALSE)
df = merge(df, demographics, by = "location_key", all = FALSE)
df = df[!duplicated(df), ]
na.omit(df)
df <- df %>% 
...
```{r}
df <- transform(df, cumulative_persons_fully_vaccinated_per_capita = cumulative_persons_fully_vaccinated / population)
df <- transform(df, cumulative_deceased_per_capita = cumulative_deceased / population)
df <- transform(df, deceased_to_confirmed = cumulative_deceased / cumulative_confirmed)
df <- transform(df, cumulative_confirmed_per_capita = cumulative_confirmed / population)
df <- transform(df, older_per_capita = ( + population_age_70_79 + population_age_80_and_older)/population)
df <- transform(df, eighty_or_older_per_capita = population_age_80_and_older / population)
df <- transform(df, population_rural_per_capita = population_rural / population)
```
...
We need more granular insight into this data so we study correlations now.

Studying the correlation between various variables in the data will give us an idea of what factors impacted the outcomes of the pandemic. 
