...
From seeing the contents and format of the data sets, we see the column names of each dataset, and deduce that the three datasets do not have identical columns, but snl_casts has common columns with snl_actors, and snl_seasons, respectively, while snl_actors, and snl_seasons do not share common columns. Although we cannot perform a straighforward, simple combining function of the three datasets using a singular function, we can join the snl_actors and snl_seasons datasets to snl_casts using two columns from snl_cast. The steps are broken down in a list for clarity:

1.Join snl_actors to snl_cast
2.Join snl_seasons dataset to the now combined dataset produced in Step 1 using the column it shares with snl_cast.

After performing a join between snl_actors and snl_cast, and snl_seasons dataset is then joined with the new, combined version of snl_cast (already combined with snl_actors), and the three datasets will be then be combined together.

# Tidy Data steps needed?

Although not every single column of the three datasets are needed - specifically one column in snl_actors, entitled "URL". When reviewing the individual data sets above, I removed the url column, and checked that the removal was successful. Retaining the rest of the columns within the data sets provides the possibilities of unanticipated analysis by keeping the columns and rows of the three datasets. Because the size of the dataset files are not extraordinarily large, we can easily keep all of the data sets' contents without burdening our coding work or analysis.
```{r}
...
### Joining two of the three data sets

Now we can create a join of the casts data set and the actors data set:
```{r}

#full join of casts and actors data sets:

casts_actors<-snl_casts%>%
...
dim(casts_actors)
```
Above, we see the combined data set resulting from the join of the casts and actors data sets.


```{r}
print(summarytools::dfSummary(casts_actors,
                        varnumbers = FALSE,
                        plain.ascii  = FALSE, 
                        style        = "grid", 
                        graph.magnif = 0.70, 
...
The combined data set resulting from the join of the casts and actors data sets has 2764 rows and 10 columns. We expected 10 columns.


Now we can create a join of the casts data set and the actors data set:
```{r}

#full join of casts and seasons data sets:

casts_seasons<-snl_casts%>%
