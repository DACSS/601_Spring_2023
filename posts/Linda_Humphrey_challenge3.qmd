...
I renamed all columns to be clear, created a response variable with 'yes' and 'no' values, and recalculated the percent variable.
```{r}
...
Here I will pivot the data, and compare our pivoted data dimensions to the dimensions calculated above as a "sanity" check.
...
# Pivot longer using tidyr
df_long <- pivot_longer(df, cols = c(yes, no), names_to = "response", values_to = "percent")
