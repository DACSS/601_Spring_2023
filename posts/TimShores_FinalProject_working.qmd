...
The big picture shows **`r crYeargrowthPctChar`% growth** in total hours worked by police officers in the two towns together **from `r year(crYearDF$year)[1]` to `r year(crYearDF$year)[5]`**. 
...
callReasonsDF %>%
  ...
  filter(call_reason %in% c(levels(topfactors[[1]]))) %>%    # this uses topfactors to target factors for removal
...
We can facet plot to zoom in on the top 9 reasons, and see how they've changed over time. This is interesting, but difficult to see a pattern with all of the variation. Some trend up, some trend down.
...
```{r chunkByReason4, fig.width=10, fig.height=4}
