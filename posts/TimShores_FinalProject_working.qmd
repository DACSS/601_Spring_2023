...
my_packages <- c("tidyverse", "fs", "pdftools", "knitr", "ggplot") # create vector of packages
...
a. `callReasonsDF` (`r ncol(callReasonsDF)` variables, `r nrow(callReasonsDF)` observations): Each observation is a call reason (examples include "Assault", "Traffic violation", "Community policing") for each year and town, with a summary count of calls initiated by the local PD, the count of calls sent to the PD from dispatch, the average time to arrive to the call (in hours), and the average time spent on the call (in hours). 
b. `callActionsDF` (`r ncol(callActionsDF)` variables, `r nrow(callActionsDF)` observations): Each observation is a call action (examples include "Investigated", "Services Rendered", "Report Taken") for each year, town, and call reason, with a summary count of call actions. Each observation has a many to one relationship with a call reason observation: each call reason observation can have many call actions, but each call action observation can have only one call reason observation.
c. `operatorSummaryDF` (`r ncol(operatorSummaryDF)` variables, `r nrow(operatorSummaryDF)` observations): Each observation is an ethnicity, race, or sex describing automobile operators for each year and town, with summary count of operators. The data does not relate these records to call reasons or call actions.

To get a sense of the scale of time spent on calls, here is a summary of time per call by the top 5 call reason mean times:
...
callReasonsDF_ReasonSummary <- callReasonsDF %>% 
  ...
  summarise(
    meanHours = mean(avg_hours_per_call, na.rm = TRUE), 
    modeHours = find_mode(avg_hours_per_call), 
    minHours = fivenum(avg_hours_per_call, na.rm = TRUE)[1], 
    lowHingeHours = fivenum(avg_hours_per_call, na.rm = TRUE)[2], 
    medianHours = median(avg_hours_per_call, na.rm = TRUE), 
    upHingeHours = fivenum(avg_hours_per_call, na.rm = TRUE)[4], 
    maxHours = fivenum(avg_hours_per_call, na.rm = TRUE)[5]
    ) %>%
  arrange(desc(meanHours))
...
Summary of time per call by town:
...
Summary of time per call by year:
...
Sanity check of call totals from source report against calculated call totals: 

```{r}

checkTotalsDF <- callReasonsDF %>%
  group_by(town) %>%
  summarise(
    selfTotal = sum(self),
    dispTotal = sum(disp)#,
    #avgArriveTotal = mean(avg_hours_to_arrive),
    #avgSceneTotal = mean(avg_hours_at_scene)
  )

checkTotalsDF <- totalReasonsDF %>%
  group_by(town) %>%
  summarise(
    selfTotal = sum(self),
    dispTotal = sum(disp)#,
...
ggplot(sumCRTownYear, aes(x = year, y = callTotal, fill = town)) + 
  geom_area() +
  labs(title = "Total police call volume by town",
       y = "Calls") +
...
sumCRTownYearTheme <- callReasonsDF %>%
  group_by(year, call_theme) %>%
    summarise(
    callTotal = sum(self) + sum(disp),
    avgHours = mean(avg_hours_to_arrive + avg_hours_at_scene),
    callTotalHours = callTotal * avgHours
  ) %>%
  arrange(desc(callTotal))
...
sumCRTownYearTheme %>% 
  ...
  ggplot(aes(x = year, y = callTotal, fill = call_theme)) +
    geom_area() +
    labs(title = "Call volume, top 5 call themes in each year", 
         y = "Calls") +
...
sumCRTownYearTheme %>% 
  ...
  ggplot(aes(x = year, y = callTotal, fill = call_theme)) +
    ...
    labs(title = "Call volume, bottom 5 call themes in each year", 
         y = "Calls") +
    ylim(0, 4000) + # same scale as first graph
...
sumCRTownYearTheme %>% 
  ...
  ggplot(aes(x = year, y = callTotal, fill = call_theme)) +
    ...
    labs(title = "Call volume, bottom 5 call themes in each year", 
         y = "Calls") +
    ylim(0, 1000) + # same order of magnitude as first graph
    scale_fill_manual(values = myPalette[c(1:5)]) # consistent colors between graphs
...
sumCRTownYearTheme %>% 
  ...
  ggplot(aes(x = year, y = callTotal, fill = call_theme)) +
    ...
    labs(title = "Call volume, bottom 5 call themes in each year", 
         y = "Calls") +
                    # lower order of magnitude than first graph
    scale_fill_manual(values = myPalette[c(1:5)]) # consistent colors between graphs
...
ggplot(aes(x = year, y = callTotalHours, fill = town)) + 
  geom_area() +
  labs(title = "Total police officer hours on all calls by town",
       y = "Hours") +
  geom_smooth(method = lm, 
              se = FALSE,
              color = "Black") +
...
sumCRTownYearTheme %>% 
  ...
  # avgHours by top 5 themes
sumCRTownYearTheme %>% 
  slice_max(order_by = avgHours, n = 5) %>%
  mutate(call_theme = fct_reorder(call_theme, avgHours, .fun = 'sum')) %>%
  ggplot(aes(x = year, y = avgHours, fill = call_theme)) +
    geom_area() +
    labs(title = "Average hours, top 5 call themes in each year", 
         y = "Hours") +
    scale_fill_manual(values = myPalette[c(6:10)]) # consistent colors between graphs

  # callTotalHours by top 5 themes
sumCRTownYearTheme %>% 
  slice_max(order_by = callTotalHours, n = 5) %>%
  mutate(call_theme = fct_reorder(call_theme, callTotalHours, .fun = 'sum')) %>%
  ggplot(aes(x = year, y = callTotalHours, fill = call_theme)) +
    geom_area() +
    labs(title = "Total hours, top 5 call themes in each year", 
         y = "Hours") +
    scale_fill_manual(values = myPalette[c(6:10)]) # consistent colors between graphs
