...
6. What can we learn about public safety need in terms of broad themes?
:::
...
callReasonsDF %>%
  ...
  labs(title = "Top growth",
        subtitle = "Top 30 call reasons by growth (Hours worked + growth)",
        x = "Hours",
        y = "Reasons") +
  #scale_fill_viridis(discrete = T, option = "cividis") +
  theme_ipsum() 
...
crTopGrowthHours %>%
  ...
  labs(title = "Leaderboard: Top growth",
       subtitle = "9 call reasons growing the most",
       y = "Hours") +
  scale_fill_viridis(discrete = T, option = "cividis") +
  theme_ipsum() +
  theme(axis.text.x = element_text(angle = 90)) +
...
crFactorTotals2 <- callReasonsDF %>%
  group_by(call_reason) %>%
    ...
    totalHours = sum(totalHours),
    estYearlyHourChange_Reason = mean(estYearlyHourChange_Reason),
    productTopHours = mean(estYearlyHourChange_Reason) * sum(totalHours)
  ) 

rm(topHourFactors) # clear the previous df

topHourFactors <- crFactorTotals2 %>%
  arrange(desc(productTopHours)) %>%
  head(n = 30) %>%
  droplevels.data.frame() # discard the empty factor levels
...
callReasonsDF %>%
  ...
  mutate(call_reason = fct_reorder(call_reason, productTopHours)) %>% 
  filter(call_reason %in% c(levels(topHourFactors[[1]]))) %>%    
      # this uses topHourFactors to target factors for removal
      # does it work in this plot?
  droplevels.data.frame() %>% 
...
Now can examine the 9 facets of the most impactful call reasons:
...
crTopGrowthHours %>%
  ...
  labs(title = "Leaderboard: Top impact",
       subtitle = "9 call reasons with top hours and top growth",
       y = "Hours") +
  scale_fill_viridis(discrete = T, option = "cividis") +
  theme_ipsum() +
  theme(axis.text.x = element_text(angle = 90)) +
...
crFactorTotals2 <- callReasonsDF %>%
  group_by(call_reason) %>%
    ...
    totalHours = sum(totalHours),
    estYearlyHourChange_Reason = mean(estYearlyHourChange_Reason),
    productBottomHours = mean(estYearlyHourChange_Reason) * sum(totalHours)
  ) 

bottomHourFactors <- crFactorTotals2 %>%
  arrange(desc(productBottomHours)) %>%
  tail(n = 30) %>%
  droplevels.data.frame() # discard the empty factor levels
...
callReasonsDF %>%
  ...
  mutate(call_reason = fct_reorder(call_reason, productBottomHours)) %>% 
  ...
  labs(title = "Bottom impact",
        subtitle = "30 call reasons with greatest negative growth 
and greatest level-of-effort",
        x = "Hours",
        y = "Reasons") +
  #scale_fill_viridis(discrete = T, option = "cividis") +
  theme_ipsum() 
...
The 9 facets of the least impactful call reasons:

```{r chunkByReason10, fig.width=10, fig.height=10}
...
crBottomGrowthHours %>%
  ...
  labs(title = "Leaderboard: Bottom impact",
       subtitle = "9 call reasons with top hours and top decrease",
       y = "Hours") +
  scale_fill_viridis(discrete = T, option = "cividis") +
  theme_ipsum() +
  theme(axis.text.x = element_text(angle = 90)) +
...
```{r chunkByReason11, fig.width=10, fig.height=4}


### HERE
### double check that lm stuff
### #2 - theme/reason
### #3 - nix the year and town group_by LM code? why did I even do that? Maybe it'll be useful later.
