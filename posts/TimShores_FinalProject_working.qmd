...
names(projectFiles_list) <- projectFiles
  ...
  # "sub" page id before the map, so I used a nested for loop to assign value to innerpageid before appending my mapped/mutated df to
...
allYearsDF_wide <- allYearsDF_wide %>%
  group_by(outerpageid, filename, innerpageid, y) %>% # I think outerpageid and filename together are redundant
...
callTypesDF <- allYearsDF_wide %>% # First df achievement unlocked
  filter(is.na(action)) %>% 
...
callActionsDF <- allYearsDF_wide %>% # second df achievement unlocked
  filter(!is.na(action)) %>%
  filter(!grepl("total", call_reason, ignore.case = TRUE)) %>%
...
operatorSummaryDF <- allYearsDF_wide %>% # third and final df achievement unlocked
  filter(!is.na(action)) %>%
  filter(grepl("total", call_reason, ignore.case = TRUE)) %>%
