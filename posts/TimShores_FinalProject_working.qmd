...
You may want to include this section to present the background information that helps readers understand more about the topic such as literature review, current discussion, major argument, or your previous knowledge. But this isn’t an academic paper, keep it short.
In a storytelling sense, you are just reminding the reader of why the answer to your question matters, or why we are interested in exploring these data.
...
Describe the “cases” represented by the dataset(s); in other words, what does each row represent?
keep it concise. You will likely merge parts 2-4 into a single section.

Dataset Description (keep it short and relevant only)
...
(optional) If you use multiple datasets, quickly describe and show how you merge and join the datasets

Briefly describe the source data and the technique for reading it in and parsing it.

```{r}

# test with
# library(tidyverse)
# alldata <- data.frame(x1 = 14:11, n_state = c(1:100))
# alldata %>% mutate(alldata, species_group = ifelse(n_state < 10, 1, ifelse(
#   n_state %in% 10:19, 2, ifelse(n_state %in% 20:29, 3, ifelse(
#     n_state %in% 30:39, 4, ifelse(n_state %in% 40:49, 5, 0)
#   ))
# )))
# alldata %>%
#   mutate(alldata,
#          species_group = case_match(
#            n_state,
#            c(1:9) ~ 1,
#            c(10:19) ~ 2,
#            c(20:29) ~ 3,
...
callActionsDF <- allYearsDF_wide %>% 
  ...
  # Third and final df achievement unlocked
...
operatorSummaryDF <- allYearsDF_wide %>% 
  ...
  rename(opid = action) %>%
  select(c(outerpageid, town, year, innerpageid, y, opcategory, opid))  %>%
