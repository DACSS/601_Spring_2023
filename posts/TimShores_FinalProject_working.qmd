...
allYearsDF <- projectFiles %>%
  map(pdf_data) %>%
  bind_rows(.id = 'filename') %>%
  mutate(filename = projectFiles[as.integer(filename)]) %>%
  mutate(filename = str_match(filename, 'posts/TimShores_FinalProject_Data/(.*)\\.pdf')[,2])
...
lev2018 <- projectFiles %>%
  ...
  # old: lev2018_wide <- lev2018[2] %>% filter(height == 11 & y > 13 & text != "Action:") %>% pivot_wider(id_cols = y, names_from = x, values_from = text) 

lev2018_wide <- lev2018 %>% 
...
############### how do i fix the next line?
############### the problem is that it's creating too many obs.
###############

lev2018_wide <- lev2018_wide %>% 
  pivot_wider(id_cols = c(row, filename, y), names_from = x, values_from = text)
#  select(-row)

# This doesn't work but it might be a good lead...
lev2018_wide <- lev2018_wide %>% 
  group_by(filename, y) %>%
  summarise(vlist = paste0(where(is.character(.)), collapse = ",")) %>%
  ungroup()
...
numcols = sort(as.numeric(names(lev2018_wide)[!grepl("[a-z]", names(lev2018_wide))]))
lev2018_wide <- select(lev2018_wide, filename, y, match(numcols, names(lev2018_wide))) # note the omission of 'rows'
  # now we have a df that is simply x:y coordinates of the page

  # time to smush cols together!
lev2018_wide <- unite(lev2018_wide, "x5to52", c("5":"47"), sep = " ", na.rm = TRUE, remove = TRUE)
