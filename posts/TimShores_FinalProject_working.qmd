...
# initialize df for reasons: group by year
crYearDF <- callReasonsDF %>%
  group_by(year) %>%
  summarise(
    totalCalls = sum(totalCalls),
    totalHours = sum(totalHours),
    estYear_HoursByReason = mean(estYear_HoursByReason) # averages estimated changes of all call reasons in each year
  ) 
...
crHoursEstByYear <- do(tempGroupDF, 
  ...
  filter(term == "year") %>%                    
  mutate(estYear_HoursByTown = round(estimate, 1)) %>%
  arrange(desc(estimate)) %>%
  select(c(town, estYear_HoursByTown))

callReasonsDF <- callReasonsDF %>%
  inner_join(crHoursEstByYear, by = join_by(town))

callReasonsDF <- callReasonsDF %>%
  filter(!is.na(estYear_HoursByTown)) 

##########

# initialize df for town: group by year
crTownYearDF <- callReasonsDF %>%
  group_by(town, year) %>%
  summarise(
    totalCalls = sum(totalCalls),
    totalHours = sum(totalHours),
    estYear_HoursByTown = mean(estYear_HoursByTown)
  ) 
...
#     totalCalls = sum(totalCalls),
#     totalHours = sum(totalHours),
#     est_AnnualChangeHours = mean(est_AnnualChangeHours)
#   ) 
# 
# crTownYearThemeDF <- callReasonsDF %>%
...
# repeating the analysis used in the previous chunks, 
# this gets factors of top 30 call reasons -- but this time, by linear regression model of change over time


# 
# testtidy %>%
#   ggplot(aes(year, .fitted)) +
#   geom_line(aes(group = totalHours), alpha = 1 / 2) + 
