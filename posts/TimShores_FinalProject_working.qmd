...
crYeargrowthLM <- as.character(round(lm(callTotalHours ~ year(year), data = crYearDF)$coefficients[2], 1))
...
# calculate growth percentage of total hours worked by town
crLevgrowth <- as.character(round((crTownYearDF[5,5] - crTownYearDF[1,5]) / crTownYearDF[1,5] * 100, 1))
crWengrowth <- as.character(round((crTownYearDF[10,5] - crTownYearDF[6,5]) / crTownYearDF[6,5] * 100, 1))
...
The big picture shows **`r crYeargrowthPct`% growth** in total hours worked by police officers in the two towns together **from `r year(crYearDF$year)[1]` to `r year(crYearDF$year)[5]`**. Another way to express this is with a linear model, the orange line: we've seen the two towns add **`r crYeargrowthLM` hours** of policing every year. What drives this growth in policing in 2 small rural towns?

The simplest next step is to compare the towns. Faceting the 3 key metrics by town lets us compare the change in calls and hours worked over the 5-year period:
...
Total hours worked from `r year(crYearDF$year)[1]` to `r year(crYearDF$year)[5]`:

Leverett: **`r crLevgrowth`% growth** 

Wendell: **`r crWengrowth`% growth**

As described earlier, I learned from the LPD Chief that Wendell's police department didn't log every call. We see a lot of growth from the change in administration, increasing the number of Wendell issues logged as calls.
