...
###
### totalCalls is getting doubled
### 2018 should be 1,998, not 3,996
### 2018 avg should be 0.584170513
### 2018 totalHours should be 1167.172685
###
# initialize df
# group by year
crYearDF <- callReasonsDF %>%
  ...
  summarise(
    ...
    avgHours = mean(avg_hours_per_call),
    totalHours = sum(totalHours)
  ) 

###
### begin calculations for crYearDF
###

# calculate growth percentage of total hours worked

crYeargrowthPct <- as.character(round((crYearDF[5,4] - crYearDF[1,4]) / crYearDF[1,4] * 100, 1))

# (values used inline to communicate growth, hence character conversions)

# calculate linear model coefficient estimate of x (hour increase by year)

crYeargrowthLM <- round(lm(totalHours ~ year(year), data = crYearDF)$coefficients[2], 1)

# calculate weekly hour increase per lm

crYeargrowthLMweek <- as.character(round(crYeargrowthLM / 52, 1))
...
crYeargrowthFTE <- as.character(round(crYeargrowthLM / 52 / assumedFTE, 2))

# convert lm coefficient of x to character

crYeargrowthLM <- as.character(crYeargrowthLM)

###
### end of calculations for crYearDF
###
...
crTownYearDF <- callReasonsDF %>%
  ...
  summarise(
    ...
    totalHours = callTotal * avgHours # needs to happen before grouping
  ) 

###
### begin calculations for crTownYearDF
###

# For each town: calculate growth percentage of hours worked

crLevgrowth <- round(
  (crTownYearDF[5,5] - crTownYearDF[1,5]) / crTownYearDF[1,5] * 100, 1)[[1]]
crWengrowth <- round(
  (crTownYearDF[10,5] - crTownYearDF[6,5]) / crTownYearDF[6,5] * 100, 1)[[1]]

# values communicated in a df, 
# hence no need for character conversions
...
# defined in previous code: assumed standard 40-hour work week
# calculate hour increase per lm in terms of weekly FTE
...
###
### end of calculations for crTownYearDF
###
### now assign to df for presentation:

crtyGrowthDF <- tribble(
  ~town, ~yr_pct_growth,  ~yr_hr_growth, ~wk_hr_growth, ~fte_growth,
  "Leverett", crLevgrowth, crLevgrowthLM, crLevgrowthLMweek, crLevgrowthFTE,
  "Wendell", crWengrowth, crWengrowthLM, crWengrowthLMweek, crWengrowthFTE
)
...
Another way to express this is with a linear model, the orange line: we've seen the two towns add **`r crYeargrowthLM` hours** of policing every year. This could be useful for the town's personnel planning. Assuming a **`r assumedFTE`-hour full-time equivalence (FTE)** and an increase of `r crYeargrowthLM` hours per year (or **`r crYeargrowthLMweek` hours per week**), Leverett and Wendell police call hours grow by **`r crYeargrowthFTE` FTE each year**.
...
Most calls come from Leverett and the LPD works most hours in response to Leverett calls. Leverett trend lines are flat: volume increases slightly, average hours per call decreases slightly, total hours is flat. Wendell average per call declines at about the same slope as Leverett, but Wendell call volume and hours show steeper incline.

Using the same analysis that I applied to total hours, this table summarizes rates of change that correspond to the orange trend lines in these plots: 
...
This reveals that Wendell, despite their low proportion of calls and hours, drives the growth in calls by a factor of around 20 to 1 compared with Leverett. 

It's more accurate and fair to say that missing data from Wendell drives an apparent growth in calls. As described above, when I discussed the data with LPD Chief Minckler, I learned that Wendell's former police chief didn't log every call. Improved data input by LPD officers has created a more accurate record of policing in Wendell. It's possible to use this data set to estimate Wendell's past police call data, but that's beyond the scope of this analysis.
...
By taking responsibility for policing in Wendell, LPD officers have improved call data about Wendell policing, bringing it up to LPD quality. When summarizing data inclusively before and after regionalization, Wendell boasts **an astounding `r as.character(crtyGrowthDF[2,2])`% annual growth** in hours on calls -- or what appears to be the addition of `r as.character(crtyGrowthDF[2,4])` hours of demand for police work every week. 
...
With `r as.character(length(levels(callReasonsDF$call_reason))[1])` factor levels, the plot of total hours by call reason is a blur:
*plus why is the shape different?*

```{r chunkByReason, fig.width=10, fig.height=10}
...
# NOT WORKING
# verified multiple times: grouping by multiple factors
# plays havoc with my total hours
...
crYearReason <- callReasonsDF %>%
  ...
  summarise(
    callTotal = sum(self) + sum(disp),
    avgHours = mean(avg_hours_to_arrive) + mean(avg_hours_at_scene),
    totalHours = callTotal * avgHours # needs to happen before grouping
  )

crYearReason %>%
  ggplot(aes(x = year, y = totalHours, fill = call_reason)) +
    geom_area(alpha = 0.5, show.legend = FALSE, color = "tomato") +
...
crThemeYear %>%
  ggplot(aes(x = year, y = totalHours, fill = call_theme)) + 
    geom_area(alpha = 0.5, show.legend = TRUE, color = "tomato") +
...
crCompareReasons %>%
  ...
  ggplot(aes(town, totalHours)) +
    geom_boxplot() +
    scale_y_continuous(limits = quantile(crCompareReasons$totalHours, c(0.05, 0.95)))
...
topfactors <- crCompareReasons %>%
  ...
  subset(totalHours < quantile(crCompareReasons$totalHours, 0.99)) %>% # complement
...
#     totalHours = sum(totalHours)
#   ) %>%
#   subset(totalHours = quantile(crCompareReasons$totalHours, c(0.98, 0.99))) %>% # complement
#   droplevels.data.frame()
# 
# # #2 this doesn't work for some reason
# crCompareReasons %>%
...
# 
#   # totalHours by bottom 5 themes
# sumCRTownYearTheme %>% 
#   slice_min(order_by = totalHours, n = 5) %>%
#   mutate(call_theme = fct_reorder(call_theme, totalHours, .fun = 'sum')) %>%    
#   ggplot(aes(x = year, y = totalHours, fill = call_theme)) +
#     geom_area() +
