...
### 2018 totalHours should be 1998 * 0.584... = 849.2
###
### Make note in analysis: not clear how the source reports handle their average time totals.
### So my interp is a guess: avgHours is the mean of the columsn together: time to arrive plus time on scene. 
### Mean denominator is the number of rows. 
### Since the report values are already averages, we don't want to use number of calls as denominator.
# initialize df
# group by year
crYearDF <- callReasonsDF %>%
  ...
  summarise(
    ...
    totalHours = sum(totalHours)                                  # this is correct
    # avgHoursTEST = sum(avg_hours_per_call) / totalCalls,        # this does not work - wrong denominator
    # avgHours = mean(avg_hours_per_call),                          # this is correct - denominator should be # of rows
    # totalHoursTEST = avgHours * totalCalls,                       # this does not work but I don't understand why
  ) 
# view(crYearDF)
# rm(crYearDF)
# write.csv(callReasonsDF,file='./callReasonsDF.csv', row.names=FALSE)
# write.csv(allYearsDF_wide,file='./allYearsDF_wide.csv', row.names=FALSE)
...
crYeargrowthPct <- as.character(round((crYearDF[5,3] - crYearDF[1,3]) / crYearDF[1,3] * 100, 1))
...
Most calls come from Leverett and the LPD works most hours in response to Leverett calls. Leverett trend lines show a slight incline. Wendell call volume and hours show a steep incline.
