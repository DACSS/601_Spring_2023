...
The *`r names(wbirds)[1]`* variable has `r nrow(wbirds) - wb_col1_null` non-null values.

The *`r names(wbirds)[2]`* variable has `r nrow(wbirds) - wb_col2_null` non-null values.

Here's the summary:

```{r}
print(dfSummary(wbirds))
```